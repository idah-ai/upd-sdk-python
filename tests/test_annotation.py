...
class TestAnnotationCreate:
    def test_create_returns_annotation(self, upd, entry):
        ...
        assert a.annotation == LABEL

    def test_create_seeds_timestamps(self, upd, entry):
        a = upd.annotations.create(entry_id=entry.id, shape_type="t", shape_args={}, annotation={})
        assert "Created-At" in a.metadata

    def test_create_with_qc_status(self, upd, entry):
        a = upd.annotations.create(
            entry_id=entry.id, shape_type="t", shape_args={}, annotation={}, qc_status="Passed"
        )
    ...
    def test_bulk_create(self, upd, entry):
        rows = [
            {"entry_id": entry.id, "shape_type": "image:bounding-box",
             "shape_args": {"x": i}, "annotation": {"id": i}}
            for i in range(5)
        ]
...
class TestAnnotationForEntry:
    def test_for_entry(self, upd, entry):
        upd.annotations.create(entry_id=entry.id, shape_type="t", shape_args={}, annotation={"n": 1})
        upd.annotations.create(entry_id=entry.id, shape_type="t", shape_args={}, annotation={"n": 2})
...
class TestAnnotationCount:
    def test_len_total(self, upd, entry):
        upd.annotations.create(entry_id=entry.id, shape_type="t", shape_args={}, annotation={})
        upd.annotations.create(entry_id=entry.id, shape_type="t", shape_args={}, annotation={})
...
class TestAnnotationQuery:
    ...
    def test_order_by_on_repo(self, upd, entry):
        upd.annotations.create(entry_id=entry.id, shape_type="zzz", shape_args={}, annotation={})
        upd.annotations.create(entry_id=entry.id, shape_type="aaa", shape_args={}, annotation={})
    ...
    def test_group_by_aggregation(self, upd, entry):
        upd.annotations.create(entry_id=entry.id, shape_type="bbox", shape_args={}, annotation={})
        upd.annotations.create(entry_id=entry.id, shape_type="bbox", shape_args={}, annotation={})
        upd.annotations.create(entry_id=entry.id, shape_type="poly", shape_args={}, annotation={})
        df = (
            upd.annotations
               .group_by("shape_type")
...
class TestAnnotationUpdate:
    def test_update_annotation_payload(self, upd, entry):
        a = upd.annotations.create(entry_id=entry.id, shape_type="t",
                                   shape_args={}, annotation={"class": "old"})
        updated = upd.annotations.update(a.id, annotation={"class": "new"})
        assert updated.annotation == {"class": "new"}
